...
public class StudentStat extends ActionSupport {
	...
	public String execute(){
		...
		List<Course> enrList = stu.getEnrolledCourses();
		boolean z = false;
		for (Course cr : enrList){
		...
		//	System.out.println(l.getLectureTitle());
		  ...
		  System.out.println("Raw: "+scripts.size());
			  ...
			  int j=0;
				...
				allQ = new ArrayList<Script>();
				j=0;
				for (int i=0; i<scripts.size(); i++){					
						if(scripts.get(i).getStudent().getEmail().equals(stu.getEmail()))
							allQ.add(scripts.get(i));
				}
				scripts=allQ;
				
				System.out.println("final filter: "+scripts.size());
